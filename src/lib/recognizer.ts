...
/**
 * Recognizer для непрерывной речи — отдаёт полный накопленный транскрипт
 * (final + interim), не дедуплицированный список слов. Для тренажёров,
 * где важна связная речь, а не отдельные слова.
 */
export function createTranscriptRecognizer(
  ...
  rec.onresult = (e) => {
    ...
    onUpdate((finalText + (interimText ? ' ' + interimText : '')).trim());
  };
